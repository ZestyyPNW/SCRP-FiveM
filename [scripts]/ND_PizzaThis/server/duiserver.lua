...
AddEventHandler('onResourceStart', function(r)
	...
	for k in pairs(duiList) do
        duiList[k].preset = duiList[k].url
    end
end)

RegisterServerEvent("ND_PizzaThis:Server:ChangeDUI", function(data)
	...
	TriggerClientEvent("ND_PizzaThis:ChangeDUI", -1, data)
end)

RegisterServerEvent("ND_PizzaThis:Server:ClearDUI", function(data)
	if data.url == "-" then duiList[data.tex].url = "-" end
	-- Clear the DUI from loading
	TriggerClientEvent("ND_PizzaThis:ClearDUI", -1, data)
	--duiList[tostring(data.tex)].url = ""
end)

lib.callback.register('ND_PizzaThis:Server:duiList', function(source)
    return duiList
end)

lib.callback.register('ND_PizzaThis:GetVehicleName', function(source, model)
    -- Get vehicle display name from model hash
