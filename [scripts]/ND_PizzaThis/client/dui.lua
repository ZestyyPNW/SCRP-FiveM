...
RegisterNetEvent('ND:characterLoaded', function(character)
	local duiList = lib.callback.await('ND_PizzaThis:Server:duiList', false)
	if inZone and duiList then
		for k, v in pairs(duiList) do TriggerEvent("ND_PizzaThis:ChangeDUI", { url = v.url, tex = k, texd = v.texd, size = v.size }) end
	end
end)
...
CreateThread(function()
	...
	PizzathisDUI:onPlayerInOut(function(isPointInside)
		if isPointInside then inZone = true
			...
			duiList = lib.callback.await('ND_PizzaThis:Server:duiList', false)
			if duiList then
				for k, v in pairs(duiList) do TriggerEvent("ND_PizzaThis:ChangeDUI", { url = v.url, tex = k, texd = v.texd, size = v.size }) end
			end
		else inZone = false
			...
			for k, v in pairs(duiList) do
				v.tex = k
				TriggerEvent("ND_PizzaThis:ClearDUI", v)
			end
			dui = {}
			duiList = {}
		end
	...
	for k, v in pairs(PresetList) do
		...
		exports.ox_target:addCircleZone({
			...
			options = {
				{
					name = "pizzathis_dui_add_"..v["tex"].texn,
					event = "ND_PizzaThis:DuiSelect",
					icon = "fas fa-circle-check",
					label = Loc[Config.Lan].targetinfo["add_image"],
					groups = JobGrade,
					tex = v["tex"].texn,
					size = v["tex"].size,
					texd = v["tex"].texd
				...
				{
					name = "pizzathis_dui_reset_"..v["tex"].texn,
					serverEvent = "ND_PizzaThis:Server:ChangeDUI",
					icon = "fas fa-circle-minus",
					label = Loc[Config.Lan].targetinfo["reset_image"],
					groups = JobGrade,
					tex = v["tex"].texn,
					texd = v["tex"].texd
				},
				{
					name = "pizzathis_dui_clear_"..v["tex"].texn,
					serverEvent = "ND_PizzaThis:Server:ClearDUI",
					icon = "fas fa-circle-xmark",
					label = Loc[Config.Lan].targetinfo["clear_image"],
					groups = JobGrade,
					tex = v["tex"].texn,
					texd = v["tex"].texd,
					url = "-"
...
RegisterNetEvent("ND_PizzaThis:DuiSelect", function(data)
	...
	local dialog = lib.inputDialog(image..Loc[Config.Lan].menu["enter_new_image"], {
		{type = 'input', label = Loc[Config.Lan].menu["html_link"], required = true}
	})
	if dialog then
		...
		if searchFound then TriggerServerEvent('ND_PizzaThis:Server:ChangeDUI', data) end
	end
end)

RegisterNetEvent("ND_PizzaThis:ChangeDUI", function(data)
	...
	if (dui[tostring(data.tex)] or inZone) and tostring(data.url) ~= "-" then
		TriggerEvent("ND_PizzaThis:ClearDUI", data)
		--Add DUI to table
...
RegisterNetEvent("ND_PizzaThis:ClearDUI", function(data)
