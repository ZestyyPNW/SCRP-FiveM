...
CreateThread(function()
	for k, v in pairs(Config.Chairs) do
		...
		exports.ox_target:addBoxZone({
			coords = v.coords.xyz,
			size = vec3(0.7, 0.7, 1.3),
			rotation = v.coords.w,
			debug = Config.Debug,
			options = {
				{
					name = "pizzathis_chair_"..k,
					event = "ND_PizzaThis:Chair",
					icon = "fas fa-chair",
					label = Loc[Config.Lan].targetinfo["sit_down"],
					loc = v.coords,
					stand = v.stand
				}
			}
...
RegisterNetEvent('ND_PizzaThis:Chair', function(data)
	local canSit = true
	local sitting = false
	local ped = PlayerPedId()

	-- Get player data from ND Framework
	local nearbyPlayers = lib.getNearbyPlayers(data.loc.xyz, 0.6, false)
	for _, v in pairs(nearbyPlayers) do
		local dist = #(GetEntityCoords(GetPlayerPed(GetPlayerFromServerId(v.id))) - data.loc.xyz)
		if dist <= 0.4 then
			lib.notify({title = "Error", description = Loc[Config.Lan].error["someone_sitting_there"], type = 'error'})
			canSit = false
		end
	end
...
AddEventHandler('onResourceStop', function(r) if r ~= GetCurrentResourceName() then return end
	for k in pairs(Chairs) do exports.ox_target:removeZone(k) end
end)
