...
function toggleItem(give, item, amount)	TriggerServerEvent("ND_PizzaThis:server:toggleItem", give, item, amount) end
...
function HasItem(items, amount)
	...
	-- Use ND Framework to get player inventory
	...
	for _, itemData in pairs(playerItems) do
		if itemData and (itemData.name == items) then
			if Config.Debug then print("^5Debug^7: ^3HasItem^7: ^2Item^7: '^3"..tostring(items).."^7' ^2Slot^7: ^3"..itemData.slot.." ^7x(^3"..tostring(itemData.count or itemData.amount).."^7)") end
			count += (itemData.count or itemData.amount or 0)
		end
	end
