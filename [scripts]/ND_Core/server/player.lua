...
function NDCore.fetchAllCharacters(src)
    ...
    if not identifier then
        print("^1[ND_Core] No identifier found for source " .. src)
        return characters
    end

    local result = MySQL.query.await("SELECT * FROM nd_characters WHERE identifier = ?", {identifier})

    if not result then
        print("^1[ND_Core] Database query failed for identifier " .. identifier)
        return characters
    end

    for i=1, #result do
        local info = result[i]
        if info and info.charid then
            local success, character = pcall(createCharacterTable, {
                source = src,
                id = info.charid,
                identifier = info.identifier,
                name = info.name,
                ...
                bank = info.bank,
                phonenumber = info.phonenumber,
                groups = json.decode(info.groups or "{}"),
                metadata = json.decode(info.metadata or "{}"),
                inventory = json.decode(info.inventory or "{}")
            })

            if success and character then
                characters[info.charid] = character
            else
                print("^1[ND_Core] Error creating character table for charid " .. info.charid .. ": " .. tostring(character))
            end
        end
    end
