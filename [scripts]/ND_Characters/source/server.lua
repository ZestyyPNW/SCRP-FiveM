...
lib.callback.register("ND_Characters:fetchCharacters", function(source)
    ...
    -- Add error handling for character fetching
    local success, result = pcall(NDCore.fetchAllCharacters, source)
    if success and result then
        characters = result
    else
        print("^1[ND_Characters] Error fetching characters for source " .. source .. ": " .. tostring(result))
    end
...
RegisterNetEvent("ND_Characters:select", function(id)
    ...
    if not characterId then
        print("^1[ND_Characters] Invalid character ID received: " .. tostring(id))
        return
    end

    local success, result = pcall(NDCore.setActiveCharacter, src, characterId)
    if not success then
        print("^1[ND_Characters] Error setting active character " .. characterId .. " for source " .. src .. ": " .. tostring(result))
    end
end)
